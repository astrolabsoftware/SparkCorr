...
//nedges
//degree
val deg=edges.groupBy("id").count.withColumnRenamed("count","degree")
dup.unpersist
...
//stats
deg.describe("degree").show()

val tdeg=timer.step
timer.print("degree")

val nsum=deg.agg(F.sum("degree")).take(1)(0).getLong(0)
...
println("Summary: ************************************")
println("@| tmax | NSIDE | Ns |  Ne  | time")
println(f"@| $tmax | $NSIDE | $Ns%g | $nsum%g | $fulltime%.2f")
println(f"@ nodes=$nodes parts=($np1 | $np2 | $np3): source=${tsource.toInt}s dups=${tdup.toInt}s join=${tjoin.toInt}s degree=${tdeg.toInt} |  tot=$fulltime%.2f mins")
